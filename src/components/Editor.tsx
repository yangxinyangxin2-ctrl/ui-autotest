...
export function Editor() {
    ...
    if (!tc) {
        return (
            <main className="w-2/5 min-w-[400px] flex flex-col border-r border-zinc-800 bg-[#4c1d95] z-0">
                <div className="min-h-[64px] flex items-center px-6 border-b border-zinc-800 bg-[#4c1d95]" />
                <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 bg-[#4c1d95]">
    ...
    return (
        <main className="w-2/5 min-w-[400px] flex flex-col border-r border-zinc-800 bg-[#4c1d95] z-0">
            {/* Editor Header */}
            <div className="min-h-[64px] flex-shrink-0 flex items-center justify-between px-6 border-b border-zinc-800 bg-[#4c1d95]">
            ...
            {/* Steps List */}
            <div className="flex-1 overflow-y-auto p-4 bg-violet-950 relative no-scrollbar">
                <ReactSortable
                    list={tc.steps}
                    setList={(newList) => {
                        // Managed by onEnd
                ...
                >
                    {tc.steps.map((step, index) => {
                        ...
                        if (!tpl) return null;
                        return (
                            <div key={step.id} className="group bg-[#5b21b6] border border-zinc-800 rounded-xl shadow-sm p-4 flex flex-col gap-3 relative pl-10 transition-all hover:shadow-md hover:border-violet-500/50">
                                {/* Drag handle */}
                                    ...
                                    {tpl.fields.map(field => {
                                        ...
                                        return (
                                            <input
                                                ...
                                                className="w-full text-sm border border-zinc-700 bg-[#4c1d95] text-zinc-200 rounded-lg px-3 py-2 outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all placeholder-zinc-600"
            ...
            {/* Add Step Bar */}
            <div className="p-4 border-t border-zinc-800 bg-[#4c1d95] flex gap-3 shrink-0">
                <select
                    value={newStepType}
                    onChange={(e) => setNewStepType(e.target.value as TestCaseStep['type'])}
                    className="flex-1 border border-zinc-700 bg-[#5b21b6] rounded-lg px-3 py-2 text-sm outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all text-zinc-200"
                >
